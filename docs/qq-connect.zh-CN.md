...
Reasonix 可以把现有的桌面端会话延伸到 QQ 上，作为远程通道使用。QQ 扩展的是当前会话，不是独立的新运行模式。
...
1. 先在当前会话里提示你输入 `App ID`
2. 再提示你输入 `App Secret`
3. 任一步输入 `/cancel` 都可以取消

这些提示和 `/qq` 结果会跟随当前应用语言切换。
...
第一次连接成功后，只要 QQ 保持启用，后续桌面会话都会自动启动 QQ 通道。
...
说明你的构建早于 QQ 支持。安装最新的 Windows 安装包，或者直接使用仓库最新 `main` 分支。
