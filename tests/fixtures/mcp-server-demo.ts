// Test-fixture demo MCP server (moved from the deleted examples/ dir).
// Minimal stdio MCP server: echo, add, get_time, slow_count tools.
// Used by tests/mcp-integration.test.ts against a real subprocess.
...
const TOOLS = [
  ...
  },
  {
    name: "slow_count",
    description:
      "Counts from 1 to n with a ~300 ms pause between steps, emitting notifications/progress frames along the way. Useful for exercising the client's progress handling.",
    inputSchema: {
      type: "object",
      properties: {
...
async function callTool(
  ...
  if (name === "slow_count") {
    ...
    // minutes. Default 5 gives ~1.5s which is plenty to see progress.
