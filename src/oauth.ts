...
/** Modern replacement — the legacy /codex/quota endpoint is being retired;
 *  the Codex CLI moved to /wham/usage (rate-limit windows + credits balance). */
...
function quotaNumber(v: unknown): number | undefined {
  if (typeof v === "number" && Number.isFinite(v)) return v;
  const n = Number(v);
  return typeof v === "string" && Number.isFinite(n) ? n : undefined;
}
...
/** Result of a quota refresh — `reason` is null on success and explains the
 *  failure otherwise, so the UI can surface it instead of a silent "—". */
export interface CodexQuotaResult {
  quota: CodexQuota | null;
  reason: string | null;
}

/** Legacy contract: absolute weekly credits (nested usage/limit objects with
 *  `amount`, or flat numeric `used`/`limit`). */
async function fetchLegacyQuota(token: string): Promise<QuotaFetchOutcome> {
  let res: Response;
  try {
    res = await fetch(CODEX_QUOTA_URL, {
      headers: {
    ...
    return { quota: null, reason: `legacy ${CODEX_QUOTA_URL}: ${(err as Error).message}` };
  }
  if (!res.ok) {
    const body = (await res.text().catch(() => "")).slice(0, 200);
    return {
      quota: null,
      reason: `${res.status} ${res.statusText} from ${CODEX_QUOTA_URL}${body ? `: ${body}` : ""}`,
    };
  }
  ...
  try {
    ...
    return {
      quota: null,
      reason: `legacy ${CODEX_QUOTA_URL}: invalid JSON — ${(err as Error).message}`,
    };
  }
  ...
  if (used === undefined || limit === undefined || limit <= 0) {
    return {
      quota: null,
      reason: `legacy payload has no used/limit fields (keys: ${Object.keys(data).join(", ") || "(empty)"})`,
    };
  }
  return {
    quota: {
...
/** Modern contract (what the Codex CLI uses today): rate-limit windows with a
 *  `used_percent` and an optional credits balance — no absolute used/limit. */
async function fetchWhamUsage(token: string): Promise<QuotaFetchOutcome> {
  let res: Response;
  try {
    res = await fetch(CODEX_WHAM_USAGE_URL, {
      headers: {
        authorization: `Bearer ${token}`,
        accept: "application/json",
    ...
    return { quota: null, reason: `wham ${CODEX_WHAM_USAGE_URL}: ${(err as Error).message}` };
  }
  if (!res.ok) {
    const body = (await res.text().catch(() => "")).slice(0, 200);
    return {
      quota: null,
      reason: `${res.status} ${res.statusText} from ${CODEX_WHAM_USAGE_URL}${body ? `: ${body}` : ""}`,
    };
  }
  let data: Record<string, unknown>;
  try {
    ...
    return {
      quota: null,
      reason: `wham ${CODEX_WHAM_USAGE_URL}: invalid JSON — ${(err as Error).message}`,
    };
  }
  const rateLimits = data.rate_limits;
  const rateLimit = (rateLimits as Record<string, unknown> | undefined)?.rate_limit;
  const primary = (rateLimit as Record<string, unknown> | undefined)?.primary_window;
  const usedPct = quotaNumber((primary as Record<string, unknown> | undefined)?.used_percent);
  if (usedPct === undefined) {
    return {
      quota: null,
      reason: `wham payload has no rate_limits.rate_limit.primary_window.used_percent (keys: ${Object.keys(data).join(", ") || "(empty)"})`,
    };
  }
  return {
    quota: { used: null, limit: null, usedPct, currency: "credits", fetchedAt: Date.now() },
    reason: "",
  };
}

/** Weekly Codex quota for the signed-in ChatGPT plan — legacy absolute
 *  credits endpoint first, then the modern wham/usage percent contract.
 *  `reason` explains a null quota for UI diagnosis. */
export async function fetchCodexQuotaDetailed(
  path: string = defaultConfigPath(),
): Promise<CodexQuotaResult> {
  const token = await resolveOpenAIToken(path);
  if (!token) {
    const reason = "not signed in with OpenAI (no OAuth token — sign in from Settings → OpenAI)";
    console.warn(`reasonix: codex quota — ${reason}`);
    return { quota: null, reason };
  }
  const legacy = await fetchLegacyQuota(token);
  if (legacy.quota) return { quota: legacy.quota, reason: null };
  const wham = await fetchWhamUsage(token);
  if (wham.quota) return { quota: wham.quota, reason: null };
  const reason = `${legacy.reason} · ${wham.reason}`;
  console.warn(`reasonix: codex quota — ${reason}`);
  return { quota: null, reason };
}
...
export async function fetchCodexQuota(
  ...
  return (await fetchCodexQuotaDetailed(path)).quota;
}
