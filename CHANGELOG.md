...
**Added — the Playwright extension ships with the Windows app and installs from the app directly.**

- The Settings → MCP page grows a "Playwright browser integration" card: one-click "Open Chrome Web Store" (the official listing, id `mmlmfjhmonkocbjadbfplnigmagldckm`), an optional Chrome-profile input feeding the `--profile-dir-name` arg (one account per Chrome profile — the multi-account story), and a Configure server button that writes the `playwright` MCP entry with `--extension` via a new `mergeMcpServerEntry` config helper (existing user entries only gain missing `--`-flags; pinned package args are never clobbered; legacy spec-string entries migrate first). Status comes from two new RPCs (`mcp_extension_status` / `mcp_extension_configure`) and a pure `computeMcpExtensionStatus` (unit-tested).
- The extension itself is bundled at packaging time — there is no upstream prebuilt artifact (source lives in the playwright monorepo, `packages/extension/`, no release zip), so `desktop/scripts/bundle-playwright-extension.mjs` (npm `bundle:extension`, wired into the release workflow after `bundle:node`) fetches the signed store CRX, strips the CRX3 envelope (magic + header length; no protobuf parsing), and extracts it into the gitignored `desktop/src-tauri/binaries/playwright-extension/`, which `tauri.windows.conf.json` maps to `<install>/assets/playwright-extension`. The daemon resolves it with the tokenizer's `here/../..` candidate pattern (`src/mcp/extension.ts`, `REASONIX_PLAYWRIGHT_EXTENSION_PATH` override) and, when present, the card offers a load-unpacked fallback (copy path / reveal folder) for offline installs — with copy stating the Web Store install is preferred (branded Chrome 137+ ignores `--load-extension`, and unpacked loads need Developer Mode).
