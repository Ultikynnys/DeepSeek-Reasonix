...
- A chat is no longer a bare `<name>.jsonl` with scattered sidecar files — it is one folder under `~/.reasonix/sessions/<name>/` holding everything about that conversation: `messages.jsonl` (the transcript), `meta.json` (summary, workspace, cost, model prefs, …), `events.jsonl`, `plan.json` and a `plans/` subfolder for completed-plan archives. Sessions are created EAGERLY: "New chat" materializes the folder and its (empty) transcript immediately, because an empty session is a real session — it lists in the sidebar, survives switches, and is only ever removed by an explicit delete. Only stray files that are not session folders at all stay invisible.
...
**Fixed: Antigravity tool requests no longer 400 on JSON-Schema `type` unions (`["string","null"]`).**

- Bridged MCP tool schemas (Outlook Mail in particular) declare nullable fields as a JSON-Schema type union, e.g. `"type": ["string", "null"]`. Gemini's `Schema.type` proto is a single enum, so an array triggered `Invalid JSON payload received. Unknown name "type" at 'request.tools[0].function_declarations[26].parameters.properties[0].value': Proto field is not repeating, cannot start list` and failed the whole request. `sanitizeGeminiSchema` now collapses a `type` union to its first concrete member and surfaces a `"null"` member as `nullable: true`, so `["string", "null"]` uploads as `{ "type": "string", "nullable": true }` at every nesting level (object properties and array `items`).
...
- The retired `deepseek-v4-flash` and `deepseek-v4-flash-vision-exp` ids stay accepted (the API routes them to V4.1 Flash) and are repriced to the Flash rate; their context window rises to 1M. The whole Flash line (`deepseek-flash`, `deepseek-v4-flash`, `deepseek-v4-flash-vision-exp`) is now vision-capable, so image attachments are no longer gated on the `-vision-exp` id alone.
- `deepseek-v4-pro` keeps its own price and stays listed; it retires 12:00 Beijing / 04:00 UTC on 2026-09-14, after which the API routes it to V4.1 Flash at the Flash price. The redirect is server-side, so there is no date-gated billing here. Pro stays text-only (no vision).
- Subagent defaults, the skill `flash` preset, and the image-gate error copy move to `deepseek-flash`; the discontinued `deepseek-chat` / `deepseek-reasoner` rows are repriced to the Flash line for stale configs.

**Fixed: an explicit "Context window" setting now raises the cap, instead of being silently clamped to the model's default.**

- Setting the Context window slider above a model's default window (e.g. 474K on a 300K DeepSeek/GPT-5.6 tab) had no effect: `resolveContextTokens` clamped every explicit `contextTokens` override to the model table's max, so the meter denominator, the fold/force thresholds, and the turn-start budget all stayed at 300K while the slider showed the value you asked for. An explicit override is now the cap, honored up to the 1M API ceiling. The per-model table (`DEEPSEEK_CONTEXT_TOKENS`) applies only when no override is set; the `[128K, 1M]` config clamp and the ceiling still bound the value. Trade-off (deliberate): an override above the model's real window now reaches the provider, so pick a value the model actually accepts. Subagent child loops are unchanged (they run without an override, so they keep the per-model default).

**Added — Playwright integration installs on demand; the base app stays lightweight.**

- The Settings → MCP page grows a "Playwright browser integration" card: one-click "Open Chrome Web Store" (the official listing, id `mmlmfjhmonkocbjadbfplnigmagldckm`), an optional Chrome-profile input feeding the `--profile-dir-name` arg (one account per Chrome profile — the multi-account story), and a Configure server button that writes the `playwright` MCP entry with `--extension` via a new `mergeMcpServerEntry` config helper (existing user entries only gain missing `--`-flags; pinned package args are never clobbered; legacy spec-string entries migrate first). Status comes from two new RPCs (`mcp_extension_status` / `mcp_extension_configure`) and a pure `computeMcpExtensionStatus` (unit-tested). The extension itself installs from the Chrome Web Store — Reasonix ships **no bundled copy**, so the base installer stays lightweight. Managed-browser modes are equally opt-in: `@playwright/mcp` is never declared as a dependency (invoked through `npx`), and browser binaries download only after an explicit "Install \<browser\>" action (`playwright_browser_install` RPC → `npx @playwright/mcp install-browser <browser>`, allowlisted to Chrome/Firefox/WebKit/Edge) — nothing Playwright-related is fetched until the user asks. A packaging invariant test asserts no Playwright package, browser cache, or extension resource enters the base manifests or Tauri resources.
- `add_mcp_server` now surfaces the catalog entry's `install_note` in its result, so the model relays per-server setup guidance (the playwright note points at `--extension` + the extension install) instead of it being dead catalog data.
- The card also takes the extension's relay token (`PLAYWRIGHT_MCP_EXTENSION_TOKEN`, password-masked input, value shown in the extension's connection dialog): stored in the entry's `env`, it skips the per-connection approval dialog that otherwise fires on every connect — the token is per-browser-profile, so one account per profile keeps its own. `mergeMcpServerEntry` now merges `env`/`headers` per-key so a stored token survives reconfiguration, while entering a new token in the card rotates it (explicit write wins over the fill). Status reports `hasToken` — never the token value itself. Pasting the connection dialog's full `PLAYWRIGHT_MCP_EXTENSION_TOKEN=…` line works as-is: the stored value is normalized to the bare token (prefix, quotes, and whitespace stripped) — the dialog's copy button copies the whole line, and storing it verbatim makes the token silently mismatch.
- The card now shows the **live connection state** of the `playwright` server, not just config state: `server connected · N tools live` (accent), `connecting…`, `server failed — <reason>` (danger), `disabled`, or `configured but not bridged yet` — derived from the same `$mcp_specs` payload that feeds the server list (no new RPC), so it updates as bridges come and go. Config state and connection state are now visibly separate: a valid-looking config with a dead or stale bridge is at least visible as "not bridged"/"failed" instead of invisible.
- The card also gains a **Test connection** button backed by a real end-to-end probe: the daemon dispatches one cheap browser tool through the live bridge with a 25s timeout and classifies the outcome (`interpretExtensionCheck`, unit-tested) — success proves the browser actually accepted the stored token; a timeout reads "no browser responded within Ns — the stored token is likely wrong, or Edge isn't running with the extension installed". The probe runs through the same registry dispatch the model uses, so a missing/disabled `browser_tabs` tool reports "not bridged" instead of a confusing error.
- **Hardcoded playwright tooling contract** (`src/mcp/playwright-tooling.ts`): any session bridging an extension-mode `@playwright/mcp` server bootstraps a durable driver + `AGENTS.md` pair into `~/.reasonix/tools/playwright/` from bundled templates (`data/tooling/playwright/`, shipped via the same `data/` packaging as the tokenizer). `driver.mjs` is a zero-dependency MCP stdio client + CLI (`list` / `open` / `call` / `seq`, abort-bounded, with teardown that auto-closes the connect-page tab each run creates — no dead tab-group litter). Upgrades are stamped (`playwright-tooling-version`): the driver is platform-managed and overwritten on bumps, while `AGENTS.md`'s platform section refreshes between markers and the agent's own notes below it are preserved (an agent-owned file without markers is never clobbered — the newest platform section lands alongside as `AGENTS.md.platform-latest.md`). Every agent driving the browser gets the duty via the bridge (`BridgeOptions.toolingNotice` on the first tool result, compact `descriptionSuffix` on every bridged description): use the driver for multi-step flows, create missing tooling, modify existing in place, keep AGENTS.md updated. The catalog note and `tests/playwright-tooling.test.ts` (12 tests: detection, bootstrap, upgrade/preserve semantics, notice + suffix injection) pin it.
...
- The bundled offline catalog gains a `playwright` entry (`@playwright/mcp`, no required user args): browser automation via accessibility snapshots, one `add_mcp_server` call away. Its note points at the `--extension` flag + Playwright MCP Chrome extension for the use case the server is most useful for here — driving your real, logged-in Chrome tabs instead of a throwaway profile.
- Server-level enable/disable had config schema, normalization, a runtime skip, and display plumbing — but no writer since the `/mcp disable` TUI command died with the desktop-only pivot. The desktop backend now accepts `mcp_specs_toggle` (two modes: `{ name, disabled }` for a server, `{ name, tool, disabled }` for a single tool), persists to config, re-bridges the tab, and re-emits `$mcp_specs`. Toggling a server with a legacy spec-string entry migrates it to a `mcpServers` object entry (carrying its `mcpEnv` overlay along, both legacy leftovers removed) — one-way, since spec strings can't express per-tool state.
- Per-tool toggles are new: `McpServerConfig.disabledTools` (bare MCP tool names, per-server so unambiguous) flows through normalization → `McpServerSpec` → `overlayMatchedSpec` → `bridgeMcpTools`, which skips disabled tools into `BridgeResult.skipped` ("disabled by user"). `reloadFromConfig` diffs the configured disable set against the live bridge and hot-unregisters/hot-re-registers only the delta — enabling/disabling one tool no longer kills the server process, and reconnects can't resurrect a disabled tool because `registerSingleMcpTool` has no caller outside the filter. A config-disabled server is now stopped on reload even though its raw spec string is unchanged (the old string-keyed reload diff couldn't see flag-only changes).
- Desktop UI: the Settings → MCP page grows an Enable/Disable button per server and a collapsible per-tool toggle list fed by the extended `$mcp_specs` payload (`disabled`, `disabledTools`, `tools`); the ContextPanel tools list renders config-disabled servers correctly before first bridge. `list_mcp_bridges` reports `disabled_tools` per bridge.
- Tests: `tests/mcp-toggle.test.ts` (migration + toggle helpers, overlay carry-over, bridge filter, runtime hot-apply incl. stop/restore and re-bridge-with-filter), `disabledTools` normalization cases in `tests/mcp-normalize-config.test.ts`, and the Playwright catalog entry in `tests/tools-scaffold.test.ts`.

**Fixed: parallel tool calls merged by the provider into one arguments string no longer die at JSON.parse.**

- When a model emits several parallel tool calls and the provider concatenates their fragments into a single `tool_call.arguments` string (observed on ollama-served models), the args looked like `{"task":"a"}{"task":"b"}` and every intended call failed with `invalid tool arguments JSON: Unexpected non-whitespace character after JSON at position N`. The repair pipeline now recognizes that shape and splits it back into one call per object (first object keeps the declared call id; the rest get derived `${id}-splitN` ids so tool results pair up; overflow beyond 8 objects is dropped and reported loudly in the repair notes). A single complete object followed by non-JSON garbage is trimmed to the leading object instead of falling to the unrecoverable-args path. Both repairs reuse the scavenger's string-aware top-level JSON scanner (`iterateJsonObjects`), so braces inside string values never truncate an object; truncated-only arguments keep the existing brace-balancing behavior unchanged. Reported as `RepairReport.argsSplitCalls` with a repair note.
- Regression tests in `tests/repair/truncation.test.ts` and `tests/repair/pipeline.test.ts` pin the split, the salvage, the string-awareness, and the untouched single-object case.
...
- Unified scheme: **turn N = the Nth real user message**. Machine-generated user records (mid-turn steers, premature-stop nudges) are now marked `synthetic` on the persisted log record — they never start a turn, never count toward the ordinal, never render as user bubbles, and are stripped before any provider request (`stripSyntheticMarkers`; non-DeepSeek payloads pass messages through verbatim, so a strict backend must not see unknown fields). The kernel's resume baseline (`resumeTurnBaseline`) counts the same real user records; `/cwd` workspace switches now reset the counter like new chats do; `retryLastUser` targets the last real prompt instead of a trailing steer; and `buildLoadedMessages` numbers every assistant record of a turn with that turn's ordinal (with a record-ordinal callId fallback) while keeping one timeline card per record so the memory-saving elision window is unchanged. Frontend render keys for assistant cards include the record index so same-turn records no longer collide.

**Added — subagent disable now applies at the knowledge level, not just at dispatch time.**

- With `enableSubagents: false` (Settings → Tools) the model used to still *see* subagents everywhere — the pinned Skills index listed `[🧬 subagent]` skills, the dedicated tools (`explore` / `research` / `review` / `security_review`) stayed in the tool spec, and the system prompt's "# Delegating to subagents via Skills" section told it to spawn — so it kept calling them and the user got `{"error":"Subagents are disabled in Settings → Tools."}` cards. The gate is now knowledge-level: `SkillStore.list()` drops `runAs: subagent` skills (custom and builtin), `registerSkillTools` skips the four dedicated spawn tools entirely, and the prompt section swaps to a short "Subagents are disabled — handle the work inline" note (`subagentsEnabled` threads `codeSystemPrompt` → `applyMemoryStack` → `applySkillsIndex`). `SkillStore.read()` stays unfiltered and the dispatch-time runner check remains the backstop, so `run_skill` on a disabled subagent skill still returns the actionable error. Inline skills (e.g. `test`) are unaffected.
- **The toggle now reaches live tabs.** `settings_save enableSubagents` previously only persisted the flag — open tabs kept advertising subagents until the workspace was reopened. The desktop's settings handler now runs a `refreshSubagentKnowledge` pass per open tab (the toolset-level twin of `syncVisionTool`): `CodeToolset.syncSubagentTools(enabled)` re-registers or unregisters the four dedicated spawn tools on the live registry (`syncDedicatedSubagentTools` — the same registration path the toolset build uses, so the two never drift), the system prompt is recomputed via `codeSystemPrompt`, and the runtime is rebuilt like a model switch so the next request carries the new prompt + tool spec.
...
- **`x-opencode-session` + `User-Agent` on every OpenCode request** (per opencode.ai/docs/go): the client sends a stable per-conversation `x-opencode-session` (the tab's session name from the desktop; one generated uuid per client for subagents/headless) and identifies as `reasonix/<version>`. Fixes "OpenCode rejected the request: Request is missing x-opencode-session and cannot be routed efficiently."
- **Premature stops no longer end the turn.** When a model that has done tool work this turn ends its reply mid-thought ("...Existing test files give me the patterns:") instead of finishing, the loop re-prompts it — bounded at 2 per turn — telling it to end with the proper completion message if the task is done, otherwise continue working now. The system prompt carries the standing rule ("never stop mid-task; end with the completion message when done"), and the give-up path is loud, never silent. Detection is a conservative heuristic (`looksLikePrematureStop`: trailing `:`/`,`/dangling bullet/unclosed code fence/narration-starter without sentence end) gated on tool activity, so plain Q&A answers are never re-prompted.
- **Output-token-truncation continuation now covers every provider**, not just Ollama: `finish_reason` `length` (OpenAI-compat/Ollama), `incomplete` (OpenCode Responses), and `MAX_TOKENS` (Gemini) all resume generation from the partial answer, bounded by the same 3-per-turn cap (renamed `MAX_TRUNCATION_CONTINUATIONS`; i18n keys `loop.truncatedContinue`/`loop.truncatedGiveUp`). The non-streaming path reads `finish_reason` from the response body too.
- **DeepSeek-family "messages[N]: missing field \`id\`" fixed at the wire layer**: `buildPayload` stamps deterministic `msg-<index>` ids on assistant/tool messages for DeepSeek-provider requests (the log never carries ids; index-derived ids keep the payload stable per log state so prompt-cache prefixes don't churn). Other providers' payloads are byte-identical to before.

**Fixed — repetition detection no longer aborts productive streams on short exact runs.**

- The stream repetition detector now requires phrase/paragraph-scale periodic runs (period > 32 chars) to be genuinely degenerate before it kills a stream: a non-dominant long-form tail must complete at least four exact cycles and exceed 1024 characters, while a substantial run that dominates the stream can still stop earlier. Healthy long-form reasoning legitimately revisits a deliberation cycle several times; those runs no longer end the turn with "produced only repetitive output" and discard the accumulated analysis. A truly stuck model's run keeps growing and re-qualifies after the fourth cycle; prefix-free degenerate streams are still caught immediately. Word-scale periods (≤ 32 chars) keep the early word-bias thresholds.
...
- Both within-stream periodic reasoning and identical reasoning across tool iterations now use the same reasoning-loop recovery helper and force-summary card lifecycle. Content and tool-call repetition still use the lightweight trim-and-stop path because they preserve a usable assistant prefix or reject malformed tool output rather than replacing reasoning context.
- Regression tests pin the detector, `streamModelResponse`, and unified loop/card behavior.
...
- **Fixed**: the gemini wire format follows the client's ENDPOINT configuration, never the model name. A client constructed with an Antigravity auth resolver routes every model through `v1internal:generateContent` — including account-specific (`chat_*`/`tab_*`) and uncataloged ids that carry no provider signal. Those ids previously fell back to the chat-completions path whenever neither the catalog nor the signed-in discovery list matched (e.g. fresh installs), which broke `inlineData` image parts and the request payload shape against the Antigravity endpoint. Regression tests pin the contract: an uncataloged id on an antigravity-configured client uses the gemini wire, and fragile discovered models still omit optional generation config.
